...
Endpoints:
  ...
  POST /audio/snap-scenes   - Silence-block-based scene cut detection (v1.0.0)
...
Changelog v1.0.0 (CLEAN REWRITE):
  - Snap-scenes algorithm completely rewritten — much simpler.
  - This release pairs with [pause] tag injection in the upstream ElevenLabs
    request preparation. The combined voiceover now contains REAL silence
    (200-300ms) between scene boundaries, generated by audio-tag directives
    that ElevenLabs v3 reliably honors.
  - With real silence present, the algorithm doesn't need peak detection,
    sustained-silence walks, or burst-aware logic. It just finds silence
    blocks and cuts in the middle.
  
  NEW ALGORITHM:
    1. Compute global speech_floor from full audio (mean_db - 15)
    2. For each scene boundary, search wide window
       [prev_end - 50ms, next_start + 600ms]
    3. Find ALL silence blocks (≥50ms consecutive chunks below floor)
    4. Pick the LONGEST silence block (= the actual sentence pause)
    5. Cut in the middle of that block
    6. Fallback (no silence found): cut at quietest chunk in window
  
  BREAKING CHANGES:
    - cut_reason values changed:
      * "silence_block_midpoint" — clean cut found (BEST, default)
      * "no_silence_block_quietest_chunk" — fallback if no silence
      * "window_too_small_midpoint" — degenerate gap
      * "no_rms_data_midpoint" — RMS measurement failed
    - Removed cut_reason values from previous versions:
      * peak_position, sustained_silence_after_ms (v0.13)
      * prev_silence_start, next_silence_end (v0.12)
    - New debug fields per cut_point:
      * silence_block_start, silence_block_end
      * silence_block_duration_ms

Changelog v0.13.0 (SUPERSEDED):
  - Tried peak detection with sustained-silence verification.
  - Failed because chunk_ms=20 was too coarse to detect peaks.
  - Even after fixing that, it was overengineered for the problem.
...
Changelog v0.11.0 (FAILED):
  - Asymmetric forward-buffer search, way too aggressive.

ROOT CAUSE OF EARLIER FAILURES (now fixed upstream):
  ElevenLabs v3 was ignoring punctuation marks in fluent speech, causing
  scene boundaries to have NO silence in many cases. No cut algorithm could
  find a clean cut where none existed in the source audio. Solution was to
  inject [pause] audio tags between sentences in the TTS request, which
  v3 reliably honors. With real silence present, snap-scenes becomes
  trivial — just find the longest silence block and cut in the middle.
...
app = FastAPI(title="FFmpeg Service", version="1.0.0")
...
@app.get("/")
def root():
    return {"service": "ffmpeg", "version": "1.0.0", "status": "running"}
...
def find_gap_quietest(
    ...
    chunk_ms: int = 10,
    back_buffer_ms: int = 50,
    forward_buffer_ms: int = 600,
    min_silence_block_ms: int = 50,
) -> dict:
    """
    Find the cut point in the gap between two scenes (v1.0.0 — clean).

    Designed to work with voiceovers that have REAL silence between scenes
    (created by [pause] audio tags in ElevenLabs). The algorithm assumes
    there will be ≥50ms of actual silence somewhere in the gap window.

    Algorithm:
      1. Search wide window: prev_end - 50ms to next_start + 600ms
         (ElevenLabs timestamps can be off by 100-200ms; we look generously)
      2. Find ALL silence blocks (≥min_silence_block_ms consecutive chunks
         below global_speech_floor_db)
      3. Pick the LONGEST silence block (= the actual sentence-break pause)
      4. Cut in the middle of that block

    Fallback if no silence block found:
      Cut at the lowest-RMS chunk in the window (best effort).
    ...
    Args:
        ...
        back_buffer_ms: search before prev_end (default 50ms)
        forward_buffer_ms: search past next_start (default 600ms)
        min_silence_block_ms: minimum block to count as a real pause

    Returns:
        dict with cut_point, cut_rms_db, gap_ms, window, chunks_measured,
        moved_from_midpoint_ms, cut_reason, silence_block_start,
        silence_block_end, silence_block_duration_ms,
        global_speech_floor_db
    """
    ...
    if window_end - window_start < 0.030:
        return {
            ...
            "silence_block_start": None,
            "silence_block_end": None,
            "silence_block_duration_ms": 0,
    ...
    min_block_chunks = max(1, min_silence_block_ms // chunk_ms)

    # Find all consecutive silence blocks (chunks below floor)
    # A block is a list of (start_idx, end_idx, length_chunks)
    silence_blocks = []
    current_start = None

    for i, (t, rms) in enumerate(windows):
        if rms < global_speech_floor_db:
            ...
            if current_start is not None:
                ...
                if length >= min_block_chunks:
                    silence_blocks.append((current_start, i, length))
                current_start = None

    # Handle silence block extending to end of window
    if current_start is not None:
        length = len(windows) - current_start
        if length >= min_block_chunks:
            silence_blocks.append((current_start, len(windows), length))

    # ============================================
    # DECIDE CUT POINT
    # ============================================
    if silence_blocks:
        # Pick the LONGEST silence block — that's the real sentence pause
        longest = max(silence_blocks, key=lambda b: b[2])
        start_idx, end_idx, length = longest

        block_start_t = windows[start_idx][0]
        block_end_t = windows[end_idx - 1][0] + chunk_s

        # Cut in the middle of the silence block
        cut_point = (block_start_t + block_end_t) / 2
        cut_reason = "silence_block_midpoint"
        block_duration_ms = round(length * chunk_ms)

        # RMS at cut point (debug)
        cut_rms = None
        ...
        return {
            ...
            "silence_block_start": round(block_start_t, 3),
            "silence_block_end": round(block_end_t, 3),
            "silence_block_duration_ms": block_duration_ms,
            "global_speech_floor_db": round(global_speech_floor_db, 2),
        }

    # ============================================
    # FALLBACK: No silence block found
    # Use the quietest chunk as cut point (best effort)
    # ============================================
    quietest = min(windows, key=lambda w: w[1])
    cut_point = quietest[0] + chunk_s / 2

    return {
        "cut_point": round(cut_point, 3),
        "cut_rms_db": round(quietest[1], 2),
        "gap_ms": gap_ms,
        "window_start": round(window_start, 3),
        "window_end": round(window_end, 3),
        "chunks_measured": len(windows),
        "moved_from_midpoint_ms": round((cut_point - gap_midpoint) * 1000),
        "cut_reason": "no_silence_block_quietest_chunk",
        "silence_block_start": None,
        "silence_block_end": None,
        "silence_block_duration_ms": 0,
        "global_speech_floor_db": round(global_speech_floor_db, 2),
...
async def snap_scenes_endpoint(
    ...
    """
    Snap scene boundaries to the longest silence block between adjacent scenes.
    
    v1.0.0 SILENCE-BLOCK ALGORITHM:
      Pairs with [pause] tag injection in TTS request. ElevenLabs v3 honors
      [pause] tags reliably, creating real 200-300ms silence between scenes.
      With actual silence present, we just find it and cut in the middle.
    
    For each pair of adjacent scenes:
      1. Compute global speech_floor from full audio (mean_db - 15)
      2. Search window: [prev_end - 50ms, next_start + 600ms]
         (generous because ElevenLabs timestamps can be off by 100-200ms)
      3. Find ALL silence blocks (≥50ms consecutive chunks below floor)
      4. Pick the LONGEST silence block (= the [pause] tag-induced gap)
      5. Cut in the middle of that block
      6. Fallback: if no silence block found, cut at quietest chunk
    
    Why this works (v1.0 design):
      - [pause] tags create unambiguous silence markers
      - The longest silence block is always the inter-scene pause
      - Mid-word silences are too short to qualify as "blocks" (filtered out)
      - No need for peak detection, sustained-silence walks, or burst logic
    
    Form params:
      file:             Combined audio file (MP3)
      scenes:           JSON array [{part, start, end}, ...]
      mode:             'gap_quietest' (DEFAULT) | 'rms_minimum' | 'silence'
      buffer_ms:        Legacy param, not used in v1.0
    ...
    Returns:
      ...
      - Per-boundary cut_reason:
        * "silence_block_midpoint" — clean silence block found (BEST, default)
        * "no_silence_block_quietest_chunk" — fallback to quietest point
        * "window_too_small_midpoint" — gap too small to analyze
        * "no_rms_data_midpoint" — RMS measurement failed
      - silence_block_start, silence_block_end, silence_block_duration_ms
        for debugging (typical [pause]-induced block: 200-300ms)
    """
    ...
    try:
        ...
        if mode == "gap_quietest":
            ...
            for i, scene in enumerate(scenes_sorted):
                ...
                if is_first:
                    ...
                    start_snap_info = {
                        ...
                        "silence_block_start": prev_cut.get("silence_block_start"),
                        "silence_block_end": prev_cut.get("silence_block_end"),
                        "silence_block_duration_ms": prev_cut.get("silence_block_duration_ms"),
                    }
                ...
                if is_last:
                    ...
                    end_snap_info = {
                        ...
                        "silence_block_start": next_cut.get("silence_block_start"),
                        "silence_block_end": next_cut.get("silence_block_end"),
                        "silence_block_duration_ms": next_cut.get("silence_block_duration_ms"),
                    }
            ...
            return {
                "status": "ok",
                "mode": "gap_quietest",
                "algorithm_version": "v1.0.0_silence_block",
                ...
                "cut_points": [
                    {
                        ...
                        "silence_block_start": cp.get("silence_block_start"),
                        "silence_block_end": cp.get("silence_block_end"),
                        "silence_block_duration_ms": cp.get("silence_block_duration_ms"),
                    }
